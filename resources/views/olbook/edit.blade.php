...
@section("content")
    ...
    <div class="content-wrap">
        ...
        <div id="title" tabindex="0"  contentEditable="true" class="input zh" placeholder="请输入标题">{{ $olbook->title }}</div>
        <div><span class="zh">「满语标题」</span></div>
        <div id="title_mnc" tabindex="0"  contentEditable="true" class="input mnc" placeholder="请输入满语标题">{{ $olbook->title_mnc }}</div>
        <div><span class="zh">「转写」</span></div>
        <div id="title_trans" tabindex="1" contentEditable="true" class="input zh" placeholder="请输入标题转写">{{ $olbook->title_trans }}</div>
        <div><span class="zh">「子标题」</span></div>
        <div id="subtitle" tabindex="2" contentEditable="true" class="input zh" placeholder="请输入子标题">{{ $olbook->subtitle }}</div>

        <div><span class="zh">「作者」</span></div>
        <div id="author" tabindex="1" contentEditable="true" class="input zh" placeholder="请输入作者">{{ $olbook->author }}</div>
        <div><span class="zh">「图书简介」</span></div>
        <div id="brief_intro" tabindex="2" contentEditable="true" class="input zh" placeholder="请输入图书简介">{{ $olbook->brief_intro }}</div>

        <form id="form" action="/olbooks/{{$olbook->id}}" enctype="multipart/form-data" method="POST">
            {{csrf_field()}}
            {{method_field("PUT")}}
